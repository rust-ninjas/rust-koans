...
impl<T: PartialEq> Node<T> {
    ...
    pub fn remove(&mut self, data: &T) {
        self.children.retain(|node| node.data != *data);
    }
...
impl<T: PartialEq> Tree<T> {
    ...
    where
        F: FnMut(&T),
    {
        fn traverse<T, F>(node: &Node<T>, visit: &mut F)
        where
            F: FnMut(&T),
        {
            node.children.iter().for_each(|child| visit(&child.data));
            node.children
                .iter()
                .for_each(|child| traverse(child, visit));
        }
        if let Some(root_node) = &self.root {
            visit(&root_node.data);
            traverse(root_node, &mut visit);
        }
    }
